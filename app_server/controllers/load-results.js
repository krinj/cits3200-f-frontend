...
module.exports.loadResults = function (req, res) {
  ...
  query[1]= "SELECT COUNT(*) AS totalResponse FROM `cits-3200.analytics.responses_dev` R  WHERE R.employment_status = '"+ employStatus +"' AND R.survey_id ='d34f3eae8e9a48b6bc8c930bc5084b87' AND R.question_id = '" + questionId + "' AND S.gender = '" + gender + "' AND R.timestamp BETWEEN '"+ startDate + "' AND '" + endDate +"' AND R.year_of_birth BETWEEN '"+ birthStart + "' AND '"+ birthEnd +"'; ";

  // Query 2: Number of completed responses for this question:
  query[2]= "SELECT COUNT(*) AS completed FROM `cits-3200.analytics.responses_dev` R  WHERE R.employment_status = '" + employStatus + "' AND R.survey_id ='d34f3eae8e9a48b6bc8c930bc5084b87' AND R.question_id = '"+ questionId +"' AND R.gender = '" + gender + "' AND R.char_count != 0 AND R.timestamp BETWEEN '"+ startDate + "' AND '" + endDate +"' AND R.year_of_birth BETWEEN '"+ birthStart + "' AND '"+ birthEnd +"'; ";

  // Query 3: Average character count:
  query[3]="SELECT AVG(char_count) AS AverageCount FROM `cits-3200.analytics.responses_dev` R  WHERE S.employment_status = '"+ employStatus +"' AND R.question_id = '"+ questionId +"' AND R.survey_id ='d34f3eae8e9a48b6bc8c930bc5084b87' AND S.gender = '" + gender + "' AND R.timestamp BETWEEN '"+ startDate + "' AND '" + endDate +"' AND R.year_of_birth BETWEEN '"+ birthStart + "' AND '"+ birthEnd +"' GROUP BY question_num;";

  // Query 4: Max character count
  ...
  // Query 5: National average overall sentiment score for the question
  query[5]= "SELECT AVG(overall_sentiment) AS overallAverage FROM `cits-3200.analytics.responses_dev` R  WHERE S.employment_status = '"+ employStatus +"' AND R.question_id = '"+ questionId +"' AND R.survey_id ='d34f3eae8e9a48b6bc8c930bc5084b87' AND S.gender = '" + gender + "' AND R.timestamp BETWEEN '"+ startDate + "' AND '" + endDate +"' AND R.year_of_birth BETWEEN '"+ birthStart + "' AND '"+ birthEnd +"' GROUP BY question_num;";

  // Query 6: Organisation's average overall sentiment score for the question
  query[6]= "SELECT AVG(overall_sentiment) AS organizationAverage FROM `cits-3200.analytics.responses_dev` R WHERE S.employment_status = '"+ employStatus +"' AND R.question_id = '"+ questionId +"' AND R.survey_id ='d34f3eae8e9a48b6bc8c930bc5084b87' AND R.gender = '" + gender + "' AND R.timestamp BETWEEN '"+ startDate + "' AND '" + endDate +"' AND R.year_of_birth BETWEEN '"+ birthStart + "' AND '"+ birthEnd +"' GROUP BY question_num;";

  // Query 7: First Date of responses for this question
  query[7]= "SELECT timestamp AS firstDate FROM `cits-3200.analytics.responses_dev` R  WHERE R.survey_id ='d34f3eae8e9a48b6bc8c930bc5084b87' AND R.question_id = '" + questionId + "' AND R.char_count != 0 AND R.timestamp ORDER BY timestamp ASC LIMIT 1;";
  ...
  for (var i= 0;i<query.length;i++){
    queryCopy[i] = query[i];
    if (gender == 'all') {
      queryCopy[i] = queryCopy[i].replace(/S.gender = 'all' AND/g, '');
    }
    if (employStatus == 'all') {
      queryCopy[i] = queryCopy[i].replace(/S.employment_status = 'all' AND/g, '');
    }
  }
